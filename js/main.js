...
for(user in userData.users){
	...
	for(trial in userData.users[user]){
		...
		if(trialObj.responseTime < 1500){
			if((trialObj.background == 1 && trialObj.face<6) || (trialObj.background == 3 && trialObj.face>6))
				(summary.incongruent).push(maxY);
			else if((trialObj.background == 3 && trialObj.face<6) || (trialObj.background == 1 && trialObj.face>6))
